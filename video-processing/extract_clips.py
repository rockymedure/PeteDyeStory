...
1. Chapter timestamps from analysis
2. Key moments identified in highlights
3. Segment-based extraction for comprehensive coverage
...
def extract_clips_from_chapters(analysis_data: dict) -> List[ClipInfo]:
    """Extract clip info from chapter breakdown in analysis"""
    clips = []
    
    synthesis = analysis_data.get('video_analysis', {}).get('synthesis_text', '')
    
    # Look for chapter patterns in synthesis
    ...
    for start, end, description in matches:
        ...
        desc = desc[:80]  # Truncate long descriptions
        ...
        if desc:
            ...
            # Limit clip duration to 3 minutes max
...
def extract_clips_from_segments(analysis_data: dict) -> List[ClipInfo]:
    """Extract clip info from segment analysis"""
    clips = []
    ...
    for segment in segments:
        ...
        # Look for interesting content markers
        priority = 0
        description = ""
        
        # High priority: Pete Dye mentions
        if re.search(r'pete\s*dye|architect', analysis.lower()):
            priority += 3
            description = "Pete Dye content"
        
        # High priority: Construction activity
        if re.search(r'construction|building|shaping|bulldozer|excavat', analysis.lower()):
            priority += 2
            description = description or "Construction activity"
        
        # Medium priority: Interviews or speaking
        if re.search(r'interview|speaking|talking|says|said', analysis.lower()):
            priority += 1
            description = description or "Interview/dialogue"
        
        # Medium priority: Opening/ceremony
        if re.search(r'opening|ceremony|celebration|grand', analysis.lower()):
            priority += 2
            description = description or "Opening/ceremony"
        
        # Medium priority: Golf course views
        if re.search(r'hole|fairway|green|bunker|tee', analysis.lower()):
            priority += 1
...
def extract_clips_from_highlights(analysis_data: dict) -> List[ClipInfo]:
    """Extract clip info from highlights section"""
    clips = []
    ...
    for num, timestamp, description in matches:
        ...
        if desc:
            clips.append(ClipInfo(
                ...
                duration="90",  # 1.5 minute default for highlights
                description=desc,
                priority=2
            ))
...
def extract_clips_from_analysis(video_path: str, output_dir: str, max_clips: int = 10) -> List[str]:
    ...
    if not os.path.exists(analysis_path):
        ...
        # Collect clips from multiple sources
        all_clips = []
        ...
        # If we don't have enough clips, add evenly spaced ones
        ...
        for clip in all_clips:
            ...
            # Check if we have a clip within 60 seconds
            too_close = False
